...
static int __world_read_file_header(struct world *world)
{
    ...
    if (binary_reader_read_uint16(world->reader, &world->num_positions) <
	0) {
	goto error;
    }
	...
	if (binary_reader_read_int32(world->reader, &world->positions[i]) <
	    0) {
	    goto error;
	}
    ...
    if (binary_reader_read_uint16(world->reader, &world->num_important) <
	0) {
	goto error;
    }
...
static int __world_read_anglers(struct world *world)
{
    ...
    world->anglers =
	talloc_array(world, char *, (uint32_t) world->num_anglers);
	...
	if ((world->anglers[i] =
	     talloc_strdup(world->anglers, angler_finished)) == NULL) {
	    _ERROR
		("%s: out of memory copying angler text from world file.\n",
		 __FUNCTION__);
...
static int __world_read_npc_killcounts(struct world *world)
{
    ...
    if (binary_reader_read_int16(world->reader, &world->num_kill_counts) <
	0) {
    ...
    if ((world->kill_counts =
	 talloc_array(world, int32_t,
		      (uint32_t) world->num_kill_counts)) == NULL) {
	...
	if (binary_reader_read_int32(world->reader, &world->kill_counts[i])
	    < 0) {
	    _ERROR
		("%s: binary reader error reading world->num_kill_counts\n",
		 __FUNCTION__);
	    ret = -1;
	    goto out;
	}
			     ...
			     uint16_t * out_tile_copies)
{
	...
	_ERROR("%s: binary reader error reading tile_flags_1",
	       __FUNCTION__);
	ret = -1;
	goto out;
    }
	...
	if (binary_reader_read_byte(world->reader, &tile_wire_flags) < 0) {
	    _ERROR("%s: binary reader error reading tile_wire_flags",
		   __FUNCTION__);
	    ret = -1;
	    goto out;
	}
	...
	if ((tile_wire_flags & 1) == 1) {
	    if (binary_reader_read_byte(world->reader, &tile_colour_flags)
		< 0) {
		_ERROR("%s: binary reader error reading tile_colour_flags",
		       __FUNCTION__);
		ret = -1;
		goto out;
	    }
	...
	if ((tile_flags_1 & WORLD_FILE_TYPE_SHORT) ==
	    WORLD_FILE_TYPE_SHORT) {
	    ret = binary_reader_read_uint16(world->reader, &tile->type);
	} else {
	    ret =
		binary_reader_read_byte(world->reader,
					(uint8_t *) & tile->type);
	}
	...
	if (world->important[tile->type] == false) {
		...
		|| binary_reader_read_int16(world->reader,
					    &tile->frame_y) < 0) {
		_ERROR("%s: binary reader error reading tile->frame_[xy]",
		       __FUNCTION__);
		ret = -1;
		goto out;
	    }
	 ...
	 * ??????
	 */
	if ((tile_colour_flags & WORLD_FILE_TILE_COLOUR) ==
	    WORLD_FILE_TILE_COLOUR) {
	    uint8_t colour;

	    if (binary_reader_read_byte(world->reader, &colour) < 0) {
		_ERROR("%s: binary reader error reading tile colour",
		       __FUNCTION__);
		ret = -1;
		goto out;
	    }
    ...
    if ((tile_flags_1 & WORLD_FILE_TILE_IS_WALL) ==
	WORLD_FILE_TILE_IS_WALL) {
	if (binary_reader_read_byte(world->reader, &tile->wall) < 0) {
	    _ERROR("%s: binary reader error reading tile->wall",
		   __FUNCTION__);
	    ret = -1;
	    goto out;
	}

	if ((tile_colour_flags & WORLD_FILE_WALL_COLOUR) ==
	    WORLD_FILE_WALL_COLOUR) {
	    uint8_t wall_colour;

	    if (binary_reader_read_byte(world->reader, &wall_colour) < 0) {
		_ERROR("%s: binary reader error reading tile->wall_colour",
		       __FUNCTION__);
		ret = -1;
		goto out;
	    }
	...
	if (binary_reader_read_byte(world->reader, &tile->liquid) < 0) {
	    _ERROR("%s: binary reader error reading tile->liquid",
		   __FUNCTION__);
	    ret = -1;
	    goto out;
	}
	...
	tile_set_wire(tile,
		      (tile_wire_flags & TILE_WIRE_1) == TILE_WIRE_1);
	tile_set_wire_2(tile,
			(tile_wire_flags & TILE_WIRE_2) == TILE_WIRE_2);
	tile_set_wire_3(tile,
			(tile_wire_flags & TILE_WIRE_3) == TILE_WIRE_3);
    }
    //TODO: tile slope

    tile_set_actuator(tile,
		      (tile_colour_flags & WORLD_FILE_TILE_COLOUR_ACTUATOR)
		      == WORLD_FILE_TILE_COLOUR_ACTUATOR);
    tile_set_inactive(tile,
		      (tile_colour_flags & WORLD_FILE_TILE_COLOUR_INACTIVE)
		      == WORLD_FILE_TILE_COLOUR_INACTIVE);
	...
	if (tile_copies != 1) {
		...
		_ERROR("%s: binary error reading tile_copies from tile",
		       __FUNCTION__);
	    }
	} else {
	    if (binary_reader_read_byte
		(world->reader, (uint8_t *) & tile_copies) < 0) {
		_ERROR("%s: binary error reading tile_copies from tile",
		       __FUNCTION__);
	    }
...
static int __world_read_tile(struct world *world)
{
    ...
    if (tile_heap_new
	(world, (uint32_t) world->max_tiles_x,
	 (uint32_t) world->max_tiles_y, &world->tiles) < 0) {
...
static int __world_read_header(struct world *world)
{
	...
	if (binary_reader_read_byte
	    (world->reader, (uint8_t *) & world->expert_mode) < 0) {
	...
	if (binary_reader_read_int64(world->reader, &world->creation_time)
	    < 0) {
	    _ERROR
		("%s: binary reader error reading world->creation_time\n",
		 __FUNCTION__);
	    ret = -1;
	    goto out;
	}
    ...
    if (binary_reader_read_byte
	(world->reader, (uint8_t *) & world->moon_type) < 0) {
	...
	|| binary_reader_read_int32(world->reader,
				    &world->tree_x[2]) < 0) {
	...
	|| binary_reader_read_int32(world->reader,
				    &world->tree_style[1]) < 0
	|| binary_reader_read_int32(world->reader,
				    &world->tree_style[2]) < 0
	|| binary_reader_read_int32(world->reader,
				    &world->tree_style[3]) < 0) {
	...
	|| binary_reader_read_int32(world->reader,
				    &world->cave_back_x[1]) < 0
	|| binary_reader_read_int32(world->reader,
				    &world->cave_back_x[2]) < 0) {
    ...
    if (binary_reader_read_int32(world->reader, &world->cave_back_style[0])
	< 0
	|| binary_reader_read_int32(world->reader,
				    &world->cave_back_style[1]) < 0
	|| binary_reader_read_int32(world->reader,
				    &world->cave_back_style[2]) < 0
	|| binary_reader_read_int32(world->reader,
				    &world->cave_back_style[3]) < 0) {
	...
	|| binary_reader_read_int32(world->reader,
				    &world->jungle_back_style) < 0
	|| binary_reader_read_int32(world->reader,
				    &world->hell_back_style) < 0) {
	...
	|| binary_reader_read_int32(world->reader,
				    &world->spawn_tile.y) < 0) {
	ret = -1;
	goto out;
    }

    if (binary_reader_read_double(world->reader, &world->world_surface) <
	0) {
    ...
    if (binary_reader_read_boolean(world->reader, &world->temp_day_time) <
	0) {
    ...
    if (binary_reader_read_int32(world->reader, &world->temp_moon_phase) <
	0) {
    ...
    if (binary_reader_read_boolean(world->reader, &world->temp_blood_moon)
	< 0) {
    ...
    if (binary_reader_read_boolean(world->reader, &world->temp_eclipse) <
	0) {
	_ERROR("%s: binary reader error reading world->temp_time\n",
	       __FUNCTION__);
	ret = -1;
    ...
    if (binary_reader_read_boolean(world->reader, &world->flags.crimson) <
	0) {
    ...
    if (binary_reader_read_boolean
	(world->reader, &world->flags.downed_boss_1) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.downed_boss_2) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.downed_boss_3) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.downed_queen_bee) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.downed_mech_1) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.downed_mech_2) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.downed_mech_3) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.downed_mech_any) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.downed_plant) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.downed_golem) < 0) {
	...
	if (binary_reader_read_boolean
	    (world->reader, &world->flags.downed_slime_king) < 0) {
	    _ERROR
		("%s: binary reader error reading world->flags.downed_slime_king\n",
		 __FUNCTION__);
	    ret = -1;
	    goto out;
	}
    ...
    if (binary_reader_read_boolean
	(world->reader, &world->flags.saved_goblin) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.saved_wizard) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.saved_mech) < 0) {
    ...
    if (binary_reader_read_boolean
	(world->reader, &world->flags.downed_goblins) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.downed_clowns) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.downed_frost) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.downed_pirates) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.shadow_orb_smashed) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.spawn_meteor) < 0) {
	_ERROR("%s: binary reader error reading world->flags.downed_*\n",
	       __FUNCTION__);
	ret = -1;
	goto out;
    ...
    if (binary_reader_read_byte(world->reader, &world->shadow_orb_count) <
	0) {
    ...
    if (binary_reader_read_boolean(world->reader, &world->flags.hard_mode)
	< 0) {
    ...
    if (binary_reader_read_int32(world->reader, &world->invasion_delay) <
	0) {
    ...
    if (world->version >= 118) {
	if (binary_reader_read_double
	    (world->reader, &world->slime_rain_time) < 0) {
	    _ERROR
		("%s: binary reader error reading world->slime_rain_time\n",
		 __FUNCTION__);
	    ret = -1;
	    goto out;
	}
	...
	if (binary_reader_read_byte
	    (world->reader, &world->sundial_cooldown) < 0) {
	    _ERROR
		("%s: binary reader error reading world->sundial_cooldown\n",
		 __FUNCTION__);
	    ret = -1;
	    goto out;
	}
    ...
    if (binary_reader_read_boolean(world->reader, &world->flags.raining) <
	0) {
	_ERROR("%s: binary reader error reading world->flags.hard_mode\n",
	       __FUNCTION__);
	ret = -1;
    ...
    if (binary_reader_read_single(world->reader, &world->cloud_bg_active) <
	0) {
	_ERROR("%s: binary reader error reading world->rain_time\n",
	       __FUNCTION__);
	ret = -1;
    ...
    if (binary_reader_read_boolean
	(world->reader, &world->flags.saved_angler) < 0) {
	_ERROR
	    ("%s: binary reader error reading world->flags.saved_angler\n",
	     __FUNCTION__);
	ret = -1;
	goto out;
    }
    ...
    if (binary_reader_read_boolean
	(world->reader, &world->flags.saved_stylist) < 0) {
	_ERROR
	    ("%s: binary reader error reading world->flags.saved_stylist\n",
	     __FUNCTION__);
	ret = -1;
	goto out;
    }
	...
	if (binary_reader_read_boolean
	    (world->reader, &world->flags.saved_tax_collector) < 0) {
	    _ERROR
		("%s: binary reader error reading world->flags.saved_tax_collector\n",
		 __FUNCTION__);
	    ret = -1;
	    goto out;
	}
	...
	if (binary_reader_read_int32
	    (world->reader, &world->invasion_size_start) < 0) {
	    _ERROR
		("%s: binary reader error reading world->invasion_size_start\n",
		 __FUNCTION__);
	    ret = -1;
	    goto out;
	}
	...
	if (binary_reader_read_int32(world->reader, &world->cultist_delay)
	    < 0) {
	    _ERROR
		("%s: binary reader error reading world->cultist_delay\n",
		 __FUNCTION__);
	    ret = -1;
	    goto out;
	}
    ...
    if (binary_reader_read_boolean
	(world->reader, &world->flags.fast_forward_time) < 0) {
	_ERROR
	    ("%s: binary reader error reading world->flags.fast_forward_time\n",
	     __FUNCTION__);
	ret = -1;
	goto out;
    }
    ...
    if (binary_reader_read_boolean
	(world->reader, &world->flags.downed_fishron) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.downed_martians) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.
				      downed_ancient_cultist) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.downed_moonlord) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.
				      downed_halloween_king) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.
				      downed_halloween_tree) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.
				      downed_christmas_ice_queen) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.
				      downed_christmas_santank) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.
				      downed_christmas_tree) < 0) {
	_ERROR("%s: binary reader error reading world->flags.downed_*\n",
	       __FUNCTION__);
	ret = -1;
	goto out;
    ...
    if (binary_reader_read_boolean
	(world->reader, &world->flags.downed_tower_solar) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.downed_tower_vortex) <
	0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.downed_tower_nebula) <
	0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.
				      downed_tower_stardust) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.active_tower_solar) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.active_tower_vortex) <
	0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.active_tower_nebula) <
	0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.
				      active_tower_stardust) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.lunar_apocalypse_up) <
	0) {
	_ERROR
	    ("%s: binary reader error reading world->flags.downed_fishron\n",
	     __FUNCTION__);
	ret = -1;
	goto out;
    }
	      ...
	      struct world **out_world)
{
	...
	_ERROR
	    ("world - could not allocate a binary reader for the world.");
	ret = -ENOMEM;
	goto failed;
    }
