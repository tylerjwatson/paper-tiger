...
static int __world_load_tile(struct world *world, uint32_t x, uint32_t y, uint16_t * out_tile_copies)
{
    ...
    tile_set_actuator(tile, (tile_colour_flags & WORLD_FILE_TILE_COLOUR_ACTUATOR)
		      == WORLD_FILE_TILE_COLOUR_ACTUATOR);
    tile_set_inactive(tile, (tile_colour_flags & WORLD_FILE_TILE_COLOUR_INACTIVE)
...
static int __world_read_header(struct world *world)
{
	...
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.downed_ancient_cultist) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.downed_moonlord) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.downed_halloween_king) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.downed_halloween_tree) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.downed_christmas_ice_queen) < 0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.downed_christmas_santank) < 0
	...
	0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.downed_tower_stardust) < 0
	|| binary_reader_read_boolean(world->reader,
	...
	0
	|| binary_reader_read_boolean(world->reader,
				      &world->flags.active_tower_stardust) < 0
...
int world_new(TALLOC_CTX * parent_context, const char *world_path, struct world **out_world)
{
    int ret = 0;
