...
static uint16_t
le16_to_cpu(const uint8_t *buf)
{
	return ((uint16_t)buf[0]) | (((uint16_t)buf[1]) << 8);
}

static uint16_t
be16_to_cpu(const uint8_t *buf)
{
	return ((uint16_t)buf[1]) | (((uint16_t)buf[0]) << 8);
}

static int
__read_7_bit_int(struct binary_reader_context *context, int32_t *out_value)
{
	int count = 0, shift = 0;
	uint8_t byte;
...
static int
__binary_reader_destructor(struct binary_reader_context *talloc_context)
{
	// struct binary_reader_context *context = talloc_get_type_abort(talloc_context, struct binary_reader_context);
	binary_reader_close((struct binary_reader_context *)talloc_context);
	return 0;
}
...
binary_reader_read_int32(struct binary_reader_context *context, int32_t *out_value)
{
	...
	if ((items = fread(&val, sizeof(int32_t), 1, context->fp)) != 1) {
		if (feof(context->fp)) {
			...
			_ERROR("%s: IO error reading file %s at position %ld\n", __FUNCTION__, context->file_path,
				   ftell(context->fp));
		};
...
int
binary_reader_read_7bit_int(const uint8_t *buffer, int *pos, int32_t *out_value)
{
	int count = 0, shift = 0;
	uint8_t byte;
