...
static int
ptGameOnPlayerLeave(const ptGame *game, const struct player *player)
{
	ptGameSendMessageToPlayer(game, player, colour_black, "%s v%d.%d.",
...
static int
ptGameOnPlayerJoin(const ptGame *game, const struct player *player)
{
	ptGameSendMessageToPlayer(game, player, colour_black, "%s v%d.%d.",
							  PRODUCT_NAME, VERSION_MAJOR, VERSION_MINOR);
	...
	console_vsprintf(game->console,
					 ...
					 player->name, player->id, player->remote_addr);

	// ptGameSendWorld(game, player);
	ptGameSyncPlayers(game, player);

	return 0;
}

int
ptGameInitialize(ptGame *game, ptGameProperties properties)
{
	int ret = -1;
/*
	...
	memcpy(game->tileFrameImportant, tileFrameImportant, sizeof(tileFrameImportant));
						  ...
						  const struct colour colour, const char *fmt, ...)
{
	...
	va_start(args, fmt);
	vsnprintf(msg, 65532, fmt, args);
	va_end(args);

	if (chat_message_new((TALLOC_CTX *)player, player, colour, msg,
						 &chat_packet) < 0) {
...
ptGameSendWorld(const ptGame *game, const struct player *player)
{
	...
	for (unsigned section = 0; section < game->world->max_sections; section++) {
		...
		if (tile_section_new(temp_context, player, section, &tile_section) <
			0) {
			_ERROR("%s: out of memory allocating status packet for world "
				   "sending.\n",
				   __FUNCTION__);
			ret = -ENOMEM;
			goto out;
